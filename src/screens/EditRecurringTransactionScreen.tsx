// src/screens/EditRecurringTransactionScreen.tsx - VERSION COMPLÈTE
...
const EditRecurringTransactionScreen = ({ route, navigation }: any) => {
  ...
  const loadTransaction = async () => {
    try {
      ...
      const transaction = await getRecurringTransactionById(transactionId);
      ...
      const hasEndDateValue = !!transaction.endDate;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Succès',
        'Transaction récurrente modifiée avec succès',
        [{ 
          text: 'OK', 
      ...
      let errorMessage = 'Impossible de modifier la transaction récurrente';
  ...
  const hasChanges = () => {
    ...
    return (
      form.description !== originalTransaction.description ||
      Math.abs(parseFloat(form.amount)) !== Math.abs(originalTransaction.amount) ||
      form.type !== originalTransaction.type ||
      form.category !== originalTransaction.category ||
      form.accountId !== originalTransaction.accountId ||
      form.frequency !== originalTransaction.frequency ||
      form.startDate.toISOString().split('T')[0] !== originalTransaction.startDate ||
      (hasEndDate && form.endDate ? form.endDate.toISOString().split('T')[0] : null) !== originalTransaction.endDate ||
      form.isActive !== originalTransaction.isActive
    );
  };

  const nextOccurrence = getNextOccurrence();

  if (initialLoading || accountsLoading || categoriesLoading) {
    return (
      <SafeAreaView>
    ...
    <SafeAreaView>
      ...
      >
        ...
        <View style={styles.statusSection}>
          ...
          <View style={styles.statusContainer}>
            <TouchableOpacity 
              style={[
                styles.statusButton,
                form.isActive && styles.statusButtonActive
              ]}
            ...
            >
              <Ionicons 
                name="play-circle" 
                size={20} 
                color={form.isActive ? '#fff' : '#34C759'} 
              />
              <Text style={[
                styles.statusButtonText,
                form.isActive && styles.statusButtonTextActive
              ]}>
                Active
              </Text>
            </TouchableOpacity>
            ...
            >
              <Ionicons 
                name="pause-circle" 
                size={20} 
                color={!form.isActive ? '#fff' : '#FF3B30'} 
              />
              <Text style={[
                styles.statusButtonText,
                !form.isActive && styles.statusButtonTextActive
        ...
        <View style={[styles.nextOccurrenceCard, isDark && styles.darkCard]}>
          ...
          {!form.isActive && (
            <Text style={[styles.inactiveWarning, isDark && styles.darkSubtext]}>
              ⚠️ Transaction inactive - ne sera pas exécutée
            </Text>
          )}
        </View>
        ...
        <View style={styles.buttonsContainer}>
          ...
          <View style={styles.actionButtons}>
            ...
            <TouchableOpacity 
              ...
              onPress={handleSave}
              disabled={loading || !form.description || !form.amount || !form.category || !form.accountId || !hasChanges()}
            >
...
export default EditRecurringTransactionScreen;